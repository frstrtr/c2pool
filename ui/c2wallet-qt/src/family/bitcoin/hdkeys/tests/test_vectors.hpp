...
// Published KAT vectors for the hdkeys module — AUTHORITATIVE sources, verbatim:
//  - BIP39: Trezor python-mnemonic vectors.json (english group); passphrase = "TREZOR".
//    tuple = {entropy_hex, mnemonic, seed_hex, bip32_root_xprv}
//  - BIP32: the BIP32 specification, Test Vector 1 (seed 000102...0f), copied
//    verbatim from bip-0032.mediawiki (xprv/xpub per chain).
//  - BIP44/49/84/86: BIP49/84/86 spec vectors for the "abandon..about" seed;
//    the BIP44 P2PKH address independently reproduced by a pure-python
//    ecdsa+base58 oracle (a second, independent implementation).
#pragma once
#include <array>
#include <string>
#include <vector>
...
struct Bip39Vec { const char* entropy; const char* mnemonic; const char* seed; const char* xprv; };
inline const std::vector<Bip39Vec>& bip39_english() {
    static const std::vector<Bip39Vec> V = {
...
struct Bip32Vec { const char* path; const char* xprv; const char* xpub; };
inline const char* bip32_tv1_seed_hex = "000102030405060708090a0b0c0d0e0f";
inline const std::vector<Bip32Vec>& bip32_tv1() {
    static const std::vector<Bip32Vec> V = {
...
struct AddrVec { const char* purpose; const char* path; const char* address; };
inline const char* addr_kat_mnemonic =
    "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about";
inline const std::vector<AddrVec>& btc_address_kats() {
    static const std::vector<AddrVec> V = {
        {"BIP44", "m/44'/0'/0'/0/0", "1LqBGSKuX5yYUonjxT5qGfpUsXKYYWeabA"},
        {"BIP49", "m/49'/0'/0'/0/0", "37VucYSaXLCAsxYyAPfbSi9eh4iEcbShgf"},
        {"BIP84", "m/84'/0'/0'/0/0", "bc1qcr8te4kr609gcawutmrza0j4xv80jy8z306fyu"},
        {"BIP84", "m/84'/0'/0'/0/1", "bc1qnjg0jd8228aq7egyzacy8cys3knf9xvrerkf9g"},
        {"BIP84", "m/84'/0'/0'/1/0", "bc1q8c6fshw2dlwun7ekn9qwf37cu2rn755upcp6el"},
        {"BIP86", "m/86'/0'/0'/0/0", "bc1p5cyxnuxmeuwuvkwfem96lqzszd02n6xdcjrs20cac6yqjjwudpxqkedrcr"},
        {"BIP86", "m/86'/0'/0'/0/1", "bc1p4qhjn9zdvkux4e44uhx8tc55attvtyu358kutcqkudyccelu0was9fqzwh"},
        {"BIP86", "m/86'/0'/0'/1/0", "bc1p3qkhfews2uk44qtvauqyr2ttdsw7svhkl9nkm9s9c3x4ax5h60wqwruhk7"},
    };
    return V;
}
