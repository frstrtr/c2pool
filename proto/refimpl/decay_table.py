...
STATUS: RATIFIED (2026-09-04). This was pinned as the CANDIDATE canonical base for M3
spec-consolidation (carry-in C2), with the normative sec.8.2 text not yet on the bridge and
any divergence from this exact-root construction standing as a [decision-needed] for
integrator before the goldens could become consensus-authoritative. That question is now
closed: the exact-root construction above is the ruled canonical base for the ratified
default lane geometry (half_life 2160, E 4096, Q62), and it is embedded in the production
consensus path at src/sharechain/v37/v37_decay_canonical.hpp — generated by
src/sharechain/v37/tools/gen_decay_canonical.py, which imports iroot_floor from THIS module
so the reference and the shipped table cannot drift into two different roots. The C++ side
gates on (half_life, epoch_len) and refuses, rather than silently downgrades, any ratified
request the embedded table cannot serve. The iterated first-order base this module was
written to replace survives there only for non-ratified test geometries.
"""
...
def iroot_floor(n, k, seed=None):
    ...
    The seed must be raised ABOVE the true root before Newton starts. Newton for the k-th
    root converges monotonically from above, and the loop below terminates the moment an
    iterate stops decreasing; handed a seed BELOW the root it therefore exits on iteration
    one having done nothing, and the closing `while (x+1)**k <= n: x += 1` correction is
    left to walk the whole remaining distance one unit at a time, each step costing a k-th
    power of a 63-bit number.

    That is not a slow path, it is a non-terminating one for this module's own caller.
    build_tables seeds InvD[j] from InvD[j-1], and InvD is INCREASING, so every single
    inverse-decay entry is seeded from below: InvD[1] alone sits 1_480_134_309_873_504
    units above its seed. Pre-bumping the seed (x += x // k + 1 until x**k >= n) costs a
    handful of powers and restores the "1-2 iterations" the docstring promises.
    """
    if k == 1:
        return n
    if n < 2:
