...
## External corroboration (independent red-team, 2026-06-27)
An independent adversarial pass over Lanes.tla re-derived the SAME hazard from a different
entry point: with incremental `scaledW` maintained by subtraction at eviction, batched
truncation (renorm) shrinks the live aggregate below the share's recorded contribution, so
`scaledW -= w * Den^(baseBin - bin)` can drive `scaledW` NEGATIVE. That negative excursion
is the non-commutativity of `floor((A-B)*lambda)` vs `floor(A*lambda)-floor(B*lambda)` seen
from the implementation side. It recommended exactly the two mitigations the spec already
encodes — a `NoNegativeScaledW` guard invariant, and modelling eviction in the post-renorm
frame (the pinned `EpochRenormalize`-before-`EvictTail` canonical order). No NEW defect:
the existing canonical-order pin + exact-per-share `epoch_rebuild()` already close it. This
strengthens, not reopens, the M3 normative clause. Suggested add: carry `NoNegativeScaledW`
into Lanes.tla as a standing invariant so any future incremental refactor re-trips the guard.
