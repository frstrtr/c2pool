...
public:
    using hash_type = typename TrackerElement::hash_type;
    using item_type = typename TrackerElement::value_type;
    using it_items = typename std::map<hash_type , item_type>::iterator;

    using fork_type = Fork<TrackerElement, 8192>;
    using fork_ptr = std::shared_ptr<fork_type>;
    using value_type = typename fork_type::value_type;
...
//                fork_by_key[value.hash()] = new_fork;
    ...
    {
        auto t0 = c2pool::debug_timestamp();
        sum_element result;

        std::shared_lock lock(mutex_);
        auto t1 = c2pool::debug_timestamp();
...
//            LOG_TRACE << "FORK: " << hash << "; " << (fork_by_key.find(hash) != fork_by_key.end());
            ...
            auto t2 = c2pool::debug_timestamp();
            result += fork->get_sum(hash);
            fork = fork->prev_fork;
            auto t3 = c2pool::debug_timestamp();
            ...
            auto t4 = c2pool::debug_timestamp();
