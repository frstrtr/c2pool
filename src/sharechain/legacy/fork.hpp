...
template <typename SumType, size_t SIZE>
class SubFork;

//template <typename CLUSTER_SUM_ELEMENT>
template <typename SumType, size_t SIZE = 8192>
class Fork : public std::enable_shared_from_this<Fork<SumType, SIZE>>
{
public:
    using cluster_type = Cluster<SumType>;
    using value_type = typename cluster_type::value_type;
    using sum_element = typename cluster_type::sum_element;
    using hash_type = uint256;
    static constexpr size_t cluster_size = SIZE;
...
{
private:
    using fork_type = Fork<SumType, SIZE>;
    using hash_type = typename fork_type::hash_type;
