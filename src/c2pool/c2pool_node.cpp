...
{
public:
    void handle_message(std::unique_ptr<RawMessage> rmsg, NodeImpl::peer_ptr peer) override { std::cout << "c2pool msg " << rmsg->m_command << std::endl;}
};

class P2Pool : public pool::Protocol<NodeImpl>
{
public:
    void handle_message(std::unique_ptr<RawMessage> rmsg, NodeImpl::peer_ptr peer) override { std::cout << "p2pool msg " << rmsg->m_command << std::endl; }
};

using Node = pool::NodeBridge<NodeImpl, P2Pool, C2Pool>;
