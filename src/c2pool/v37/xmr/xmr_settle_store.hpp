...
//
// ===========================================================================
// src/c2pool/v37/xmr/xmr_settle_store.hpp   (Track A2 / Milestone A — W6 seam)
//
// The DURABLE side of the live XMR node: an ISettleStore seam + a file-backed
// concrete store + a RecoveryDriver that rebuilds the OWED ledger + settlement
// high-water on restart, BEFORE the engine starts (donor lifecycle order).
//
...
//   ISettleBatch/ISettleStore seam, the §2.3 record codecs, census_open (F2),
//   RecoveryDriver (§4 steps 1-9), and a LevelDBSettleStore concrete binding.
//   That header is NOT on master yet (it is a draft branch). To keep THIS
//   milestone building on a fresh master clone and DIGEST-NEUTRAL, this file
//   provides a small, self-contained store that:
//     * uses ONLY merged master types (OwedLedger, SettleHW from
//       src/c2pool/v37/w4_settlement.hpp — SettleHW already models the LevelDB
//       high-water round-trip via serialize()/deserialize());
//     * mirrors the W6 ISettleStore SHAPE (batch()/get()/for_each_prefix,
//       put/remove/commit_sync) so production swaps in LevelDBSettleStore with a
//       one-line change to XmrNode (see GAP note in xmr_node.hpp);
//     * honours the two W6 red-team invariants that matter to Milestone A:
//         F1 — the finalize driver replays on_block_finalized per height, IN
//              ORDER, with the persisted per-height bin_height (never the tip);
//         F2 — fail-closed open: a torn / partially-written record aborts the
//              open rather than silently starting from an empty ledger.
//   NO consensus digest lives here: the store persists W4's OWED events and the
//   high-water only; owed_digest / lane digest are recomputed by the merged
//   OwedLedger / executor, byte-identically to a fresh run.
// ===========================================================================
#pragma once
...
// this with LevelDBSettleStore (v37/w6-persistence); Milestone A backs it with
// FileSettleStore below and MemSettleStore in tests.
// ---------------------------------------------------------------------------
struct ISettleBatch {
...
struct SettleEvent {
    ...
    std::string   bid;                 // Monero block id (hex) that carried the settlement
...
// ORPHAN events dispose. Any torn record throws → open() returns false so the
