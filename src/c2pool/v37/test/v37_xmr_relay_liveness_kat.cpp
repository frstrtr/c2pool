...
//       (a pre-0x45 build) is NOT dropped for silence (kept > 3 silence
...
int main() {
    ...
    // ── L1 codec ────────────────────────────────────────────────────────────
    {
        ...
        C(f.size() == 10 && f[0] == 0x45 && f[1] == kFbVersion && f[2] == 0x88 && f[9] == 0x11,
          "L1 PING = 45 01 | u64 LE nonce (10 B): " + hex(f));
        u8 op = 0; u64 n = 0;
        C(decode_ping(f, op, n) && op == FB_PING && n == 0x1122334455667788ull, "L1 PING round-trips");
        const auto g = encode_ping(FB_PONG, 7);
        C(g.size() == 10 && g[0] == 0x46 && decode_ping(g, op, n) && op == FB_PONG && n == 7, "L1 PONG = 46 01 | nonce, round-trips");
        ...
        C(is_family_b_opcode(FB_PING) && is_family_b_opcode(FB_PONG), "L1 0x45/0x46 inside the Family-B namespace (a pre-0x45 node counts them fb_unknown)");
#else
