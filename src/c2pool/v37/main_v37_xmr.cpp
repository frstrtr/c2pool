        ...
        if (relay_enabled()) {
            ...
            }
            {
                ...
                std::printf("relay: GAP-2 receipt relay UP listen=%s:%u peers=%zu bind=%s order=%s(L=%llu grace=%ums) share_diff=%llu "
                            ...
                            s ? (unsigned long long)s->next_pos : 0ULL, s ? hex_of(s->digest).substr(0, 12).c_str() : "-",
                            rxp->describe().c_str());
                std::printf("relay: RELAY-LIVENESS keepalive=%u ms silence-timeout=%u ms (FB_PING/FB_PONG 0x45/0x46; a silent link is dropped + redialed)\n",
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        // accounting's nomination -- because two flags could disagree.
            ...
            std::printf(
                ...
                "  --relay-keepalive-ms MS      PING every relay link this often (default 5000; 0 = off, pre-0x45 wire)\n"
