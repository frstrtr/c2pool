...
//
// LTC addrme self-probe KAT, issue #1716 (the sibling half of #882).
//
// #915 fixed the DASH Legacy addrme self-probe only. ltc's Legacy handler
// (src/impl/ltc/protocol_legacy.cpp) still compared the peer's source against
...
// gossiped that loopback record onward in an addrs message. The LTC canonical,
// p2pool-merged-v36/p2pool/p2p.py:295 (handle_addrme, master 9200d594),
// compares `host == '127.0.0.1'`, and so does ltc's own Actual generation
// (protocol_actual.cpp:57). Legacy is the generation live on ltc today.
//
// These cases use REAL types, not mocks. They drive the real
// ltc::Legacy::handle(message_addrme) and ltc::Actual::handle(message_addrme)
...
// as the btc half of #1716 (src/impl/btc/test/legacy_addrme_self_probe_test.cpp),
// which was ported from the DASH #882 KAT (test/test_dash_addrme.cpp).
//
...
//
// FOLDED into the EXISTING allowlisted ltc share_test target. A standalone
...
//
// ltc::Legacy / ltc::Actual are abstract on their own: the raw-frame entry
...
public:
    ...
    std::map<uint64_t, ltc::NodeImpl::peer_ptr>& peers() { return this->m_peers; }
};

using ProbeLegacy = Probe<ltc::Legacy>;
using ProbeActual = Probe<ltc::Actual>;

ltc::NodeImpl::peer_ptr make_socket_peer(LoopbackPair& pair, StubCommunicator& stub)
{
...
LoopbackOutcome drive_loopback_addrme(std::uint16_t advertised_port)
{
    ...
    for (int i = 0; i < 32; ++i)
    {
        auto raw = ltc::message_addrme::make_raw(advertised_port);
        gen.handle(ltc::message_addrme::make(raw->m_data), peer);
    }
...
class LtcLegacyAddrme : public ::testing::Test
{
protected:
    void SetUp() override
    {
        ...
        m_dir = std::filesystem::temp_directory_path()
              / ("c2pool-1716-ltc-addrme-" + std::to_string(::getpid()) + "-" + std::to_string(seq++));
...
TEST_F(LtcLegacyAddrme, LoopbackPeerPresentsHostAddressNotNetworkAddress)
{
    LoopbackPair pair;
    StubCommunicator stub;
...
// THE DEFECT. Canonical (p2pool-merged-v36 p2p.py:295) takes the self-probe
...
// (p2p.py:296-297). Before the fix, ltc Legacy sent it down the else arm:
...
TEST_F(LtcLegacyAddrme, LoopbackAddrmeIsNotRecordedAsARoutablePeer)
{
    ...
    EXPECT_NE(find_frame(out.frames, "addrme"), nullptr)
        << "canonical relays the addrme onward (p2pool-merged-v36 p2p.py:296-297)";
}
...
TEST_F(LtcLegacyAddrme, ActualGenerationAgreesOnLoopback)
{
...
TEST_F(LtcLegacyAddrme, RoutableSourceIsRecordedAndRelayedWithByteIdenticalFields)
{
    constexpr std::uint16_t kAdvertisedPort = 9326;                     // ltc sharechain P2P port
    ...
    for (int i = 0; i < 32; ++i)
    {
        auto raw = ltc::message_addrme::make_raw(kAdvertisedPort);
        legacy.handle(ltc::message_addrme::make(raw->m_data), peer);
    }
    ...
    auto parsed = ltc::message_addrs::make(ps);
