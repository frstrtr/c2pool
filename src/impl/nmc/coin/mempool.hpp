...
///
/// Re-homed mirror of src/impl/btc/coin/mempool.hpp into namespace nmc::coin so
/// the NMC coin tree stays self-contained (no btc::coin symbols). Namecoin is a
/// Bitcoin fork: same wire format, same 21M money cap, 100-block maturity.
/// Phase 2 + UTXO fee computation
///
...
// Namecoin shares Bitcoin's consensus money params: MAX_MONEY = 21M * 1e8 sat,
// 100-block coinbase maturity, no pegout. Reference: namecoin-core amount.h
...
public:
    ...
    explicit Mempool(core::coin::ChainLimits limits = NMC_LIMITS,
    ...
    bool add_tx(const MutableTransaction& tx, core::coin::UTXOViewCache* utxo) {
        ...
        if (m_pool.size() % 100 == 0 || m_pool.size() <= 5) {
            LOG_INFO << "[EMB-NMC] Mempool: size=" << m_pool.size()
    ...
    void remove_for_block(const BlockType& block) {
            ...
            for (const auto& vin : mtx.vin) {
                ...
                if (it != m_spent_outputs.end()) {
                    ...
                    if (m_pool.count(conflict_txid)) {
                        LOG_INFO << "[EMB-NMC] Mempool: removing conflict tx "
        ...
        if (removed > 0 || conflicts > 0) {
            LOG_INFO << "[EMB-NMC] Mempool: block cleanup removed=" << removed
                     << " conflicts=" << conflicts
    ...
    int recompute_unknown_fees(core::coin::UTXOViewCache* utxo) {
        ...
        if (resolved > 0 || still_unknown > 0) {
            LOG_INFO << "[EMB-NMC] Mempool fee revalidation: resolved=" << resolved
    ...
    int revalidate_inputs(core::coin::UTXOViewCache* utxo) {
        ...
        if (!to_remove.empty()) {
            LOG_INFO << "[EMB-NMC] Mempool revalidate: evicted " << to_remove.size()
