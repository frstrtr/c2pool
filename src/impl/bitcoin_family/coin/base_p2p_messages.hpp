...
// and dashcore/src/netaddress.h (CNetAddr::SerializeV2Stream / BIP155Network).
//
// Per-record wire layout:
//   time        uint32 LE
//   services    CompactSize (varint)       ← differs from addr's fixed uint64
//   networkID   uint8  (BIP155Network)
//   addr        CompactSize-prefixed bytes (size validated against networkID)
//   port        uint16 BE
//
// Message: CompactSize record-count (≤1000) + records.

enum class BIP155Network : uint8_t {
    IPV4  = 0x01,
    IPV6  = 0x02,
    TORV2 = 0x03,   // deprecated per BIP 155; reject on receive
    TORV3 = 0x04,
    I2P   = 0x05,
    CJDNS = 0x06,
};

inline constexpr size_t bip155_address_size(uint8_t network_id)
{
    switch (static_cast<BIP155Network>(network_id)) {
        case BIP155Network::IPV4:  return 4;
        case BIP155Network::IPV6:  return 16;
        case BIP155Network::TORV2: return 10;
        case BIP155Network::TORV3: return 32;
        case BIP155Network::I2P:   return 32;
        case BIP155Network::CJDNS: return 16;
    }
    return 0;
}
...
struct btc_addrv2_record_t
{
    ...
    C2POOL_SERIALIZE_METHODS(btc_addrv2_record_t)
    {
        READWRITE(obj.m_time,
                  ...
                  obj.m_network_id,
                  obj.m_addr,
                  Using<IntType<16, true>>(obj.m_port));
    }
};

BEGIN_MESSAGE(addrv2)
    MESSAGE_FIELDS
    (
        (std::vector<btc_addrv2_record_t>, m_addrs)
    )
    {
        READWRITE(obj.m_addrs);
    }
