...
public:
    ...
    NodeImpl(boost::asio::io_context* ctx, config_t* config)
        : base_t(ctx, config),
          m_share_getter(ctx,
            ...
            {
                auto rmsg = btc::message_sharereq::make_raw(req_id, hashes, parents, stops);
    ...
    void request_shares(uint256 id, peer_ptr peer,
                        std::vector<uint256> hashes, uint64_t parents,
                        std::vector<uint256> stops,
                        std::function<void(btc::ShareReplyData)> callback)
    {
    ...
    void got_share_reply(uint256 id, btc::ShareReplyData shares)
    {
        try { m_share_getter.got_response(id, shares); }
        catch (const std::invalid_argument&) { /* request already timed out */ }
    }

    std::vector<btc::ShareType> handle_get_share(std::vector<uint256> hashes, uint64_t parents, std::vector<uint256> stops, NetService peer_addr);
...
public:
    ...
    ADD_HANDLER(addrs, btc::message_addrs);
    ADD_HANDLER(addrme, btc::message_addrme);
    ADD_HANDLER(ping, btc::message_ping);
    ADD_HANDLER(getaddrs, btc::message_getaddrs);
    ADD_HANDLER(shares, btc::message_shares);
    ADD_HANDLER(sharereq, btc::message_sharereq);
    ADD_HANDLER(sharereply, btc::message_sharereply);
    ADD_HANDLER(bestblock, btc::message_bestblock);
    ADD_HANDLER(have_tx, btc::message_have_tx);
    ADD_HANDLER(losing_tx, btc::message_losing_tx);
    ADD_HANDLER(remember_tx, btc::message_remember_tx);
    ADD_HANDLER(forget_tx, btc::message_forget_tx);
};
