...
void Legacy::HANDLER(addrme)
{
    ...
    // NETWORK address. Canonical p2p.py:281 compares `host == '127.0.0.1'`, and
    // so does the Actual generation (protocol_actual.cpp:57). Legacy shipped
    // "127.0.0.0", which no peer can present as its source IP, so this branch
    // was unreachable and every addrme, including one arriving over loopback,
    // fell into the else arm below: it recorded 127.0.0.1:<port> in our
    // AddrStore as if it were a routable peer and then gossiped that loopback
    // record onward in an addrs message. #915 fixed the same line on DASH only.
...
{
    try {
        btc::HandleSharesData result; //share, txs
        ...
        for (auto wrappedshare : msg->m_shares)
        {
            ...
            try
            {
                share = btc::load_share(wrappedshare, peer->addr());
            }
...
void Legacy::HANDLER(sharereq)
{
    ...
    try
    {
        ...
        auto reply_msg = message_sharereply::make_raw(msg->m_id, btc::ShareReplyResult::good, rshares);
...
void Legacy::HANDLER(sharereply)
{
    ...
    if (msg->m_result == ShareReplyResult::good)
    {
        ...
        for (auto& rshare : msg->m_shares)
        {
            try
            {
                auto share = btc::load_share(rshare, peer->addr());
