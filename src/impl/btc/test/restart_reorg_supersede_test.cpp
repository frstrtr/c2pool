...
// Restart-reorg supersede detector -- KAT, exercised through BTC's REAL
// ShareTracker API (btc::ShareTracker::compute_supersede_hint).
//
// LIVE EVIDENCE this pins (contabo BTC, warm restart on the shared v36
...
//
// Folded into the EXISTING allowlisted `btc_share_test` target (never a standalone
...
uint256 build_run(btc::ShareTracker& tracker, int32_t count,
    ...
    for (int32_t i = 0; i < count; ++i) {
        ...
        uint256 h = hx(buf);
        auto* sh = new btc::MergedMiningShare();
        sh->m_hash = h;
...
void mark_verified(btc::ShareTracker& tracker, const uint256& tip) {
...
uint256 verify_deep_prefix(btc::ShareTracker& tracker, const uint256& tip, int n) {
...
TEST(BtcRestartReorgSupersede, HigherWorkForkActivates) {
    btc::ShareTracker t;
...
TEST(BtcRestartReorgSupersede, LowerWorkForkStaysInactive) {
    btc::ShareTracker t;
...
TEST(BtcRestartReorgSupersede, EqualWorkForkStaysInactive) {
    btc::ShareTracker t;
    const uint256 inc_tip = build_run(t, 30, uint256(), 1);
    mark_verified(t, inc_tip);
...
TEST(BtcRestartReorgSupersede, HealthySingleHeadIsNoOp) {
    btc::ShareTracker t;
...
TEST(BtcRestartReorgSupersede, OwnChainExtensionIsNotAChallenger) {
    btc::ShareTracker t;
    const uint256 inc_tip = build_run(t, 30, uint256(), 1);
    mark_verified(t, inc_tip);
...
TEST(BtcRestartReorgSupersede, NoIncumbentIsInactive) {
    btc::ShareTracker t;
...
TEST(BtcRestartReorgSupersede, BackfillTargetIsTwiceChainLengthPlusTen) {
    const int32_t CL = static_cast<int32_t>(btc::PoolConfig::chain_length());
    EXPECT_GT(CL, 0);
...
TEST(BtcRestartReorgSupersede, ChallengerNotStarvedByShorterHigherWorkHead) {
    btc::ShareTracker t;
...
TEST(BtcRestartReorgSupersede, PrioritizeIsNoOpWhenInactive) {
    btc::ShareTracker t;
    const uint256 inc_tip = build_run(t, 30, uint256(), 1);
    mark_verified(t, inc_tip);
    ...
    btc::SupersedeHint inactive;  // active == false
