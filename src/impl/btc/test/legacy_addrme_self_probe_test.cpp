...
//
// BTC addrme self-probe KAT, issue #1716 (the sibling half of #882).
//
// #915 fixed the DASH Legacy addrme self-probe only. btc's Legacy handler
// (src/impl/btc/protocol_legacy.cpp) still compared the peer's source against
...
// gossiped that loopback record onward in an addrs message. Canonical
// p2pool-dash/p2pool/p2p.py:281 (handle_addrme) compares `host == '127.0.0.1'`,
// and so does btc's own Actual generation (protocol_actual.cpp:57). Legacy is
// the generation live on btc today.
//
// These cases use REAL types, not mocks. They drive the real
// btc::Legacy::handle(message_addrme) and btc::Actual::handle(message_addrme)
...
// path). They read the frames the handler actually put on the wire. Ported
...
//
// FOLDED into the EXISTING allowlisted btc_share_test target. A standalone
...
//
// btc::Legacy / btc::Actual are abstract on their own: the raw-frame entry
...
public:
    ...
    std::map<uint64_t, btc::NodeImpl::peer_ptr>& peers() { return this->m_peers; }
};

using ProbeLegacy = Probe<btc::Legacy>;
using ProbeActual = Probe<btc::Actual>;

btc::NodeImpl::peer_ptr make_socket_peer(LoopbackPair& pair, StubCommunicator& stub)
{
...
LoopbackOutcome drive_loopback_addrme(std::uint16_t advertised_port)
{
    ...
    for (int i = 0; i < 32; ++i)
    {
        auto raw = btc::message_addrme::make_raw(advertised_port);
        gen.handle(btc::message_addrme::make(raw->m_data), peer);
    }
...
class BtcLegacyAddrme : public ::testing::Test
{
protected:
    void SetUp() override
    {
        ...
        m_dir = std::filesystem::temp_directory_path()
              / ("c2pool-1716-addrme-" + std::to_string(::getpid()) + "-" + std::to_string(seq++));
...
TEST_F(BtcLegacyAddrme, LoopbackPeerPresentsHostAddressNotNetworkAddress)
{
    LoopbackPair pair;
    StubCommunicator stub;
...
// THE DEFECT. Canonical (p2p.py:281) takes the self-probe arm for a loopback
// sender: it does NOT record the address and relays the addrme onward so a peer
// that can see our public address answers it (p2p.py:282-283). Before the fix,
// btc Legacy sent it down the else arm: got_addr(127.0.0.1:port) put loopback
// in the AddrStore as a routable peer (where get_good_peers() would hand it to
// other nodes in an addrs reply) and gossiped it as an addrs message.
TEST_F(BtcLegacyAddrme, LoopbackAddrmeIsNotRecordedAsARoutablePeer)
{
    ...
    EXPECT_NE(find_frame(out.frames, "addrme"), nullptr)
        << "canonical relays the addrme onward (p2p.py:282-283)";
}
...
TEST_F(BtcLegacyAddrme, ActualGenerationAgreesOnLoopback)
{
...
TEST_F(BtcLegacyAddrme, RoutableSourceIsRecordedAndRelayedWithByteIdenticalFields)
{
    constexpr std::uint16_t kAdvertisedPort = 9333;                     // btc sharechain P2P port
    ...
    for (int i = 0; i < 32; ++i)
    {
        auto raw = btc::message_addrme::make_raw(kAdvertisedPort);
        legacy.handle(btc::message_addrme::make(raw->m_data), peer);
    }
    ...
    auto parsed = btc::message_addrs::make(ps);
