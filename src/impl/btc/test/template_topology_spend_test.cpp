...
// ---------------------------------------------------------------------------
// btc_template_topology_spend (rides btc_share_test) -- #1437 KAT.
//
// Pins btc::coin::Mempool::get_sorted_txs_with_fees() as a producer of a
...
// block body, bitcoind rejects the whole template
...
// tx in feerate order". Rides the already-allowlisted btc_share_test target
// (#143 NOT_BUILT trap). p2pool-merged-v36 surface: NONE. Per-coin isolation:
// src/impl/btc/ only.
// ---------------------------------------------------------------------------
...
// Parent P (low feerate) funds child C (high feerate). Pure feerate order visits
// C first -> the OLD selector emitted [C, P] (child before parent), which
// bitcoind rejects. The fix defers C until P is selected, so both ship with the
// parent first.
TEST(BtcTemplateTopologySpend, CpfpChildOrderedAfterParent)
{
    Mempool mp;
    ...
    // Regression witness: the child out-feerates the parent, so a pure
    // highest-feerate-first pass (the old admission) emits C before P.
    ...
    // Both ship, and the parent is strictly before the child.
...
// Parent P has an unknown fee (BTC never fills unknown-fee txs), so it can never
// enter the template. The child C spends P's output. The OLD selector admitted C
// anyway (parent merely present in the pool), shipping a child WITHOUT its
// parent. The fix excludes C entirely.
TEST(BtcTemplateTopologySpend, ChildWithoutSelectableParentExcluded)
{
    Mempool mp;
    ...
    ASSERT_TRUE(mp.add_tx(P));   // fee UNKNOWN -> never selected (not in feerate index)
    ASSERT_TRUE(mp.add_tx(C));
    mp.set_tx_fee(cid, 8000);    // child fee known

    auto [sel, fees] = mp.get_sorted_txs_with_fees(4'000'000);

    // Regression witness: C is a fee-known tx whose only input funds from an
    // in-mempool parent that is NOT in the template -> the old code shipped it.
    auto s = txid_set(sel);
...
// D1 and D2 spend the SAME outpoint (CONFIRMED_X:0). m_spent_outputs keeps only
// the last writer, so both stay selectable; the OLD selector admitted BOTH,
// producing a double-spending block body. The fix ships only the higher-feerate
// spender and drops the second.
TEST(BtcTemplateTopologySpend, IntraTemplateDoubleSpendPruned)
{
    Mempool mp;
    ...
    // The shared outpoint is consumed exactly once across the whole set.
    int consumers = 0;
...
// A mix of a clean tx, a double-spend pair, and an orphan-parent child. The
// returned set must (i) contain no two txs sharing an input, (ii) place every
// in-mempool parent before its child, and (iii) yield a tx-merkle computed over
// EXACTLY the pruned set (excluded txids never enter a leaf).
TEST(BtcTemplateTopologySpend, PrunedSetIsValidBodyAndMerkleMatches)
{
    Mempool mp;

    // Clean standalone tx (confirmed input we cannot see -> admitted).
    ...
    // Double-spend pair on CONFIRMED_X:0.
    MutableTransaction D1 = pay(CONFIRMED_X, 0, 100000);
    MutableTransaction D2 = pay(CONFIRMED_X, 0, 90000);
    uint256 d1 = compute_txid(D1), d2 = compute_txid(D2);

    // Orphan-parent child: parent has unknown fee (never selected).
    ...
    auto s = txid_set(sel);
    // Excluded: the losing double-spender and the orphan child.
    EXPECT_EQ(s.count(d2), 0u);
    EXPECT_EQ(s.count(ocid), 0u);
    EXPECT_EQ(s.count(opid), 0u);
    // Included: clean tx + winning double-spender.
    ...
    // and stable, and computing it over the (invalid) full set would differ.
    ...
    with_bad.push_back(d2);      // had the double-spender leaked in...
    with_bad.push_back(ocid);    // ...or the orphan child
