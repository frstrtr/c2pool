...
public:
    ...
    // before think(). Guardrails: strictly-greater only, genuine-fork only
    ...
    // Phase-2 head order (stable_partition, so intra-group order is preserved)
    // so a shorter non-challenger verified head cannot spend the whole normal
    // budget and trip the outer per-head gate before the challenger's iteration
    // is entered. Inactive supersede => predicate is always false => no-op and
    // the healthy-node order is byte-identical. Returns the number moved.
                             ...
                             const SupersedeHint& supersede = SupersedeHint{})
    {
        ...
        // reorg stack). The exclusive tracker lock is held for the whole of
        // think(); the elevated verify pool (SupersedeHint) could otherwise draw
        // hundreds of verifies in one cycle and pin the lock for seconds,
        // degrading the IO serve path to EMPTY replies. Cap the per-cycle hold;
        // m_think_needs_continue makes run_think() repost the remainder next tick.
        // Bootstrap is exempt (no verified chain to serve yet; initial full
        // verify runs uncapped, matching p2pool's synchronous initial sync).
        ...
        // the work-descending order so a shorter non-challenger head cannot spend
        // the normal budget and trip the outer per-head gate before the
        // challenger's iteration is entered. No-op when supersede is inactive.
        ...
        for (auto& [head_hash, tail_hash] : sorted_vheads)
        {
            ...
            // missing-parent). Hoisted above the per-head budget gate so the gate
            // can be elevated-budget-aware (below).
            ...
            // the bounded elevated pool. When supersede is inactive is_challenger
            // is always false, so this reduces to the original break (byte-identical).
            ...
            // parents until the segment reaches 2*CL+10 depth so it is no longer
            // pinned at ~CL+8 (the stuck state).
