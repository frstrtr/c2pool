...
{
        auto rmsg = btc::message_ping::make_raw();
...
{
    auto rmsg = btc::message_version::make_raw(
        btc::PoolConfig::ADVERTISED_PROTOCOL_VERSION,
        1,                                    // services
        addr_t{1, peer->addr()},              // addr_to (the remote)
        addr_t{1, NetService{"0.0.0.0", btc::PoolConfig::P2P_PORT}}, // addr_from (us)
        m_nonce,
        m_software_version,
...
std::optional<pool::PeerConnectionType> NodeImpl::handle_version(std::unique_ptr<RawMessage> rmsg, peer_ptr peer)
{
    LOG_DEBUG_POOL << "handle message_version";
        std::unique_ptr<btc::message_version> msg;
        msg = btc::message_version::make(rmsg->m_data);
...
void NodeImpl::download_shares(peer_ptr /*unused_peer*/, const uint256& target_hash)
{
    ...
    std::weak_ptr<pool::Peer<btc::Peer>> weak_peer = peer;
    auto peer_addr_for_log = peer->addr();

    request_shares(req_id, peer, hashes, parents, stops,
        [this, weak_peer, target_hash, peer_addr_for_log, req_id](btc::ShareReplyData reply)
        {
            m_downloading_shares.erase(target_hash);
            m_pending_share_reqs.erase(req_id);
...
void NodeImpl::load_persisted_shares()
{
    ...
    for (size_t i = skip; i < total_in_db; ++i)
    {
        ...
        try
        {
            ...
            auto share = btc::load_share(static_cast<int64_t>(ver), ps, NetService{"database", 0});
...
void NodeImpl::run_think()
{
    ...
    boost::asio::post(m_think_pool, [this, block_rel_height]() {
      ...
      try {
        ...
        // restart soak can be verified from logs.
                ...
                try {
                    ...
                    for (auto [hash, data] : view) {
                        data.share.invoke([&](auto* s) {
                            if (s->m_stale_info == btc::StaleInfo::orphan) ++orphan_count;
                            else if (s->m_stale_info == btc::StaleInfo::doa) ++doa_count;
                        });
                        ++total_recent;
                    }
                } catch (...) {}
...
void NodeImpl::clean_tracker()
{
    ...
    boost::asio::post(m_think_pool, [this]() {
      ...
      try {
        ...
        auto CL = static_cast<int32_t>(btc::PoolConfig::chain_length());
    ...
    if (!m_last_top5_heads.empty())  // p2pool node.py:359: if decorated_heads:
    {
        ...
        for (int iter = 0; iter < 1000; ++iter)
        {
            ...
            for (auto& [head_hash, tail_hash] : heads_copy)
            {
                ...
                // eaten every clean and restarted forever. Matched by segment id.
    ...
    // ensures only shares far beyond the PPLNS window are removed.
    {
        ...
        for (int iter = 0; iter < 1000; ++iter)
        {
            ...
            for (auto& [tail_hash, head_hashes] : tails_copy)
            {
                ...
                // segment (it needs its FULL ~2*CL depth to reach CHAIN_LENGTH
                // verified shares; trimming mid-verification silently restarts
                // convergence). Exemption lifts once it verifies to CL and the
