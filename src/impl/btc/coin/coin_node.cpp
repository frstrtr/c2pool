...
{
    // No work source configured at all -> empty view (1:1 with ltc reference).
    if (!m_embedded && !m_rpc)
        return {};
...
bool CoinNode::submit_block_hex(const std::string& block_hex, bool ignore_failure)
{
    ...
    // BTC's NodeRPC::submit_block_hex is already the agnostic 2-arg/no-mweb
    // form (MWEB is LTC-specific and absent here), so we forward directly --
    // no 3-arg mweb="" coin-side overload exists or is needed for BTC.
