...
public:
        ...
        bool*              underfill_tripped = nullptr)
    {
        ...
        // only (no deployments to signal). NEVER inherit tip.header.m_version --
        // a mainnet tip carries another miner's BIP310-rolled bits inside the
        // 0x1fffe000 rolling mask; advertising them makes ESP-Miner's submitted
        // version_bits (rolled & ~job_version) lossy, so no formula can recover
        // the hashed version and every share rejects as a random hash.
