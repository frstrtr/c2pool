...
// reads share-chain metadata — so the port is a 1:1 mirror of btc::PoolMonitor
// with the namespace swapped to bch. The only chain-specific touch is the
// address formatter (see check_concentration): BCH uses base58 legacy /
// CashAddr addressing, NOT litecoin, so script_to_address is called with
// is_litecoin=false.
    ...
    int check_concentration(ShareTracker& tracker, const uint256& best, int32_t height)
    {
        ...
        for (auto& [label, depth] : windows)
        {
            ...
            for (auto& [addr, work] : addr_work)
            {
                ...
                // BCH: base58 legacy / CashAddr — not litecoin — so is_litecoin=false.
                std::string addr_str = core::script_to_address(
                    addr, false /*is_litecoin*/, PoolConfig::is_testnet);
                if (addr_str.empty()) {
