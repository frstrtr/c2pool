...
/// BIP 152 Compact Block Support -- ported from src/impl/btc/coin/compact_blocks.hpp.
///
...
///
/// >>> BCH DIVERGENCES <<<
///  * NO SegWit (M1 4.1): BCH has no witness data. The BTC source wrapped every
///    transaction body in TX_WITH_WITNESS(...); here txs serialize directly via
///    the legacy Transaction format (transaction.hpp banner). UnserializeTransaction
///    is the 2-arg (tx, stream) form -- there is no TxParams/witness flag.
///  * Short IDs over txid (BIP152): BTC v2 keys short IDs on wtxid. BCH has no
///    wtxid distinction (wtxid == txid), so BuildCompactBlock computes short IDs
///    via compute_txid(). The reconstruction lookup is keyed by the caller's
///    mempool.all_txs_map_wtxid() -- which on BCH aliases all_txs_map() (txid).
///    That alias is the seam; it stays at the call site and is not threaded here.
///  * CTOR (Nov 2018) is a template-builder / validation ordering rule, NOT
///    enforced by these carriers. Reconstruction preserves the index order it is
...
struct CompactBlock {
    ...
    void Serialize(Stream& s) const {
        ...
        for (size_t i = 0; i < prefilled_txns.size(); ++i) {
            ...
            ::Serialize(s, pt.tx);  // BCH: legacy (no-witness) tx body
            prev_index = pt.index;
        }
    }
    ...
    void Unserialize(Stream& s) {
        ...
        for (size_t i = 0; i < n_prefilled; ++i) {
            ...
            UnserializeTransaction(prefilled_txns[i].tx, s);  // BCH: 2-arg, no-witness
...
struct BlockTransactionsResponse {
    ...
    void Serialize(Stream& s) const {
        ...
        for (const auto& tx : txs)
            ::Serialize(s, tx);  // BCH: legacy (no-witness) tx body
    }
    ...
    void Unserialize(Stream& s) {
        ...
        txs.resize(n);
        for (auto& tx : txs)
            UnserializeTransaction(tx, s);  // BCH: 2-arg, no-witness
    }
};
...
/// BCH: short IDs are computed over txid (compute_txid) -- wtxid == txid.
                                       ...
                                       uint64_t nonce = 0)
{
    ...
    // BCH: short IDs keyed on txid (no wtxid distinction).
    if (txs.size() > 1) {
        uint64_t k0, k1;
...
/// @param known_txs Map of txid -> transaction. BCH has no wtxid distinction, so
///                  the caller supplies mempool.all_txs_map_wtxid() (== all_txs_map).
