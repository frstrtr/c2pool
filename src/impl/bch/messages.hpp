...
// BCH share/pool p2p protocol messages.
//
// Mirrors btc::messages — the on-wire p2pool protocol is coin-independent and
// must stay byte-compatible with frstrtr/p2pool-merged-v36 (p2pool/p2p.py).
//
// BCH divergence from btc: BCH has NO SegWit / witness. coin::Transaction and
// coin::MutableTransaction serialize without a marker/flag byte or witness
// stack (see coin/transaction.hpp), so message_remember_tx writes its txs
// plainly — there is no TX_WITH_WITNESS wrapper on this chain.
...
// message_remember_tx
// BCH: no witness — serialize MutableTransaction plainly (no TX_WITH_WITNESS).
