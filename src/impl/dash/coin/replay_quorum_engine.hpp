...
inline std::optional<NewQuarterOutput> build_new_quarter_members(
    ...
    bool skip_removed_mns = true, uint32_t dbg_h = 0)
{
    ...
    const bool dbg_on = (dbg_h == 1800288u);
    if (dbg_on) {
        // NotUsed portion size = combined - (used set score-sorted). We tag
        // each entry by whether it is a used MN so the diff can see the
    ...
    if (dbg_on) {
        for (size_t i = 0; i < out.quarters.size(); ++i) {
            std::string line;
            for (MnRef m : out.quarters[i])
                line += m->proTxHash.GetHex().substr(0,12) + ",";
            LOG_INFO << "[RNEWQ] i=" << i << " [" << line << "]";
        }
    }
    ...
    bool skip_removed_mns = true, uint32_t dbg_h = 0)
{
    ...
    auto built = rotdetail::build_new_quarter_members(
        ...
        previous, skip_removed_mns, dbg_h);
