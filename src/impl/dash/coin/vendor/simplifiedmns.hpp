...
struct CSimplifiedMNListEntry
{
    ...
    SERIALIZE_METHODS(CSimplifiedMNListEntry)
    {
        READWRITE(obj.nVersion,
                  obj.proRegTxHash,
                  obj.confirmedHash,
                  Using<RawBytesFormat<NETADDR_SIZE>>(obj.netAddress),
                  Using<BigEndianFormat<2>>(obj.netPort),
                  Using<RawBytesFormat<BLS_PUBKEY_SIZE>>(obj.pubKeyOperator),
                  obj.keyIDVoting,
                  obj.isValid);
        if (obj.nVersion >= VER_BASIC_BLS) {
            READWRITE(obj.nType);
            if (obj.nType == TYPE_EVO) {
                if (obj.nVersion < VER_EXT_ADDR) {
                    READWRITE(Using<BigEndianFormat<2>>(obj.platformHTTPPort));
                }
