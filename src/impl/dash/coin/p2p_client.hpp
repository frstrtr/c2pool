...
public:
    ...
    void handle(std::unique_ptr<RawMessage> rmsg, const NetService& service) override
    {
        ...
        } catch (const std::out_of_range&)
        {
            ...
            const bool first = m_unhandled_seen.insert(cmd).second;
            if (first) {
                LOG_WARNING << "[" << m_chain_label << "] DROPPED unhandled p2p command '"
                            << cmd << "' cause=not_in_handler_set value="
                            << (rmsg ? rmsg->m_data.size() : 0) << "B (first occurrence; further "
                ...
                LOG_DEBUG_COIND << "[" << m_chain_label << "] ignoring unhandled command '"
                                << cmd << "' (" << (rmsg ? rmsg->m_data.size() : 0) << " bytes)";
            }
            return;
