...
namespace dash
{
// DASH note: namespace-port of btc::ShareTracker. DASH is X11 (PoW-independent
// of btc SHA256d); the tracker is consensus-agnostic — share weighting, PPLNS
// windows and version-gating are PoW-independent, so the port is structural.
// Cross-coin aux chain-id diagnostics retained for the v36-standardized shape.
...
public:
    ...
    // inline static: header-self-sufficient (DASH has no node.cpp yet; the
    // pool-node leaf adds it). btc/dgb define these out-of-line in node.cpp —
    // converging that packaging cross-coin is a later Bucket-2 standardization.
    inline static std::vector<uint64_t> s_decay_table;
    inline static uint64_t s_decay_per = 0;
    inline static bool s_table_initialized = false;
...
public:
    // Coin params (X11 pow_func + targets) used by attempt_verify's Phase-1
    // share_init_verify. Set by the node at construction; default-empty for the
    // PoW-independent window/decay KATs, which never reach attempt_verify.
