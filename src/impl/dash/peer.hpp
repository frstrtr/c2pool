...
//
// S8 pool-node leaf 5 (FINAL) slice A bundles the per-peer state struct so the
// NodeImpl skeleton in node.hpp can name dash::Peer as the BaseNode PeerData
// template argument. Mirrors btc::Peer / dgb::Peer: the per-peer state a pool
// Node keeps for every connected p2pool share-network peer (NOT a dashd
...
// share-message protocol (remote_txs = hashes the peer advertises;
// remembered_txs = full txs we have forwarded/learned, keyed by hash).
//
