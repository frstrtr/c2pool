...
/// DGB p2pool share-gossip message types (pool-layer wire protocol).
///
/// Phase B B2 (pool-node layer). Ported verbatim from src/impl/ltc/messages.hpp,
/// Scrypt-only — DGB pool gossip is byte-identical to LTC. The DGB-specific
/// network differs only in config_pool/config_coin params, not message layout.
/// COMPAT: byte-identical-except-namespace keeps wire parity with
