...
// DGB share verification (Phase B B1, Scrypt-only).
// Ported from src/impl/ltc/share_check.hpp into the dgb:: namespace. LTC is the
// Scrypt parent: PoW is scrypt(1024,1,1,256) for both, share format / SEGWIT
// activation (ver 17) / witness-commitment nonce are byte-identical to the
// p2pool-merged-v36 reference, so the verification surface ports verbatim with
// only the namespace swapped. Fenced conformance constants (exact PPLNS,
// min-proto 3301, AutoRatchet 95/50/2x) and the F10 version-acceptance path live
// in config_pool / auto_ratchet (pillar-1 gate), not here.
// COMPAT: frstrtr/p2pool-merged-v36; any divergence => [decision-needed].
// DECISION-NEEDED watch: the DASH third-invariant work-neutral continuity-header
// rule is NOT encoded in the LTC Scrypt parent, so it is absent from this verbatim
// port. Continuity-header handling for DGB is deferred and surfaced to integrator
// rather than resolved inline (see reply 2026-06-16).
