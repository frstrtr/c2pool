...
// DGB scriptPubKey -> human-readable address using DGB's own Base58/bech32
// version bytes (CoinParams). The ltc source used the is_litecoin convenience
// overload, which only knows litecoin/bitcoin — DGB must pass explicit params.
inline std::string dgb_script_to_address(const std::vector<unsigned char>& script)
{
    const bool tn = PoolConfig::is_testnet;
    return core::script_to_address(
        script,
        tn ? CoinParams::TESTNET_BECH32_HRP      : CoinParams::BECH32_HRP,
        tn ? CoinParams::TESTNET_ADDRESS_VERSION : CoinParams::ADDRESS_VERSION,
        tn ? CoinParams::TESTNET_P2SH_VERSION    : CoinParams::ADDRESS_P2SH_VERSION);
}
    ...
    int check_concentration(ShareTracker& tracker, const uint256& best, int32_t height)
    {
        ...
        for (auto& [label, depth] : windows)
        {
            ...
            for (auto& [addr, work] : addr_work)
            {
                ...
                // Convert scriptPubKey to human-readable address (DGB encoding)
                std::string addr_str = dgb_script_to_address(addr);
                if (addr_str.empty()) {
            ...
            if (cycle_ % 10 == 0)
            {
                ...
                for (auto& [a, w] : sorted_addrs)
                {
                    if (count >= 3) break;
                    if (count > 0) top3 << " ";
                    std::string display = dgb_script_to_address(a);
                    if (display.empty()) {
