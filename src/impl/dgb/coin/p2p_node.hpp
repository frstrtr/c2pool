...
#define ADD_P2P_HANDLER(name)\
    void handle(std::unique_ptr<dgb::coin::p2p::message_##name> msg)
namespace dgb
{
namespace coin
{
...
public:
    NodeP2P(io::io_context* context, dgb::interfaces::Node* coin, config_t* config,
    ...
    void submit_block(BlockType& block)
    {
        if (m_peer)
        {
            auto rmsg = dgb::coin::p2p::message_block::make_raw(block, {});
            m_peer->write(rmsg);
        } else
        {
