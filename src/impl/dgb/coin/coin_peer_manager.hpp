...
/// DGB-ISOLATED multi-peer manager for the embedded coin-daemon P2P arm.
///
/// This is the network-standalone gate for a fully daemonless c2pool-dgb:
/// it lets an embedded DigiByte-network arm reach the DGB network on its OWN
/// scored, group-diverse peer set — independent of the local digibyted — so
...
/// and the precondition for disabling the external digibyted entirely.
///
...
/// the sibling dash::coin::DashCoinPeerManager, NOT a reuse of the shared
/// merged tree. Each single-coin leaf keeps its own copy to preserve the
/// per-coin isolation fence (DGB lives entirely under src/impl/dgb, no coupling
...
///
/// Header-only to match the sibling dgb coin leaves (coin/*.hpp).
...
struct DgbPeerInfo
{
    ...
    bool                is_protected{false};    // local digibyted node — never drop
    ...
    int compute_score() const
    {
        ...
        // This is what pulls the embedded arm OFF the local digibyted's peer view
...
struct DgbPeerManagerConfig
{
    ...
    std::set<uint16_t> valid_ports;             // only connect to peers on known ports (DGB: 12024/12026)
...
// ─── DgbCoinPeerManager ──────────────────────────────────────────────────────
class DgbCoinPeerManager
{
public:
    using GetPeerInfoFn = std::function<std::vector<NetService>()>;

    DgbCoinPeerManager(boost::asio::io_context& ioc,
                       const std::string& symbol,
                       const std::string& data_dir,
                       const DgbPeerManagerConfig& config)
        : m_ioc(ioc)
        , m_symbol(symbol)
        , m_data_dir(data_dir)
        , m_config(config)
    ...
    ~DgbCoinPeerManager() { stop(); }

    /// Set callback to get peers from digibyted via RPC getpeerinfo (optional).
    void set_getpeerinfo_fn(GetPeerInfoFn fn) { m_getpeerinfo_fn = std::move(fn); }

    /// Register the protected/pinned local digibyted node. Score=999999, never
    ...
    void start()
    {
        ...
        LOG_INFO << "[" << m_symbol << "] DgbCoinPeerManager started: "
    ...
    const DgbPeerManagerConfig& config() const { return m_config; }
...
private:
                             ...
                             bool require_routable)
    {
        ...
        // don't re-add the digibyted's own peers under a preferred source.
        auto key = ep->to_string();
        if (m_peers.count(key)) return false;
        if (source != DgbPeerInfo::Source::coind && m_coind_peers.count(key)) return false;

        // ── Network group limits (Sybil resistance) ──
        auto group = peer_network_group(ep->host());
        bool is_untried_source = (source == DgbPeerInfo::Source::addr_crawl
                                || source == DgbPeerInfo::Source::dns_seed
                                || source == DgbPeerInfo::Source::fixed_seed);
    ...
    void bootstrap_from_getpeerinfo()
    {
        ...
        try {
            ...
            for (auto& addr : peers) {
                ...
                if (try_add_peer_locked(addr, DgbPeerInfo::Source::coind,
    ...
    std::string db_path() const
    {
        ...
        if (m_data_dir.empty() || m_data_dir == ".")
            dir = ::core::filesystem::config_path() / "dgb_embedded_peers";
        else
            dir = m_data_dir;
        ...
        if (!clean) {
            LOG_WARNING << "[DgbPeerManager] non-alphanumeric coin symbol '"
        ...
        try {
            ...
            if (j.contains("peers") && j["peers"].is_object()) {
                ...
                for (auto& [key, pj] : j["peers"].items()) {
                    ...
                    peer.source = static_cast<DgbPeerInfo::Source>(
                        pj.value("source", 0));
    ...
    void bootstrap_from_dns_seeds()
    {
        ...
        for (auto& addr : peers) {
            ...
            if (try_add_peer_locked(addr, DgbPeerInfo::Source::dns_seed,
    ...
    void load_fixed_seeds()
    {
        ...
        for (auto& addr : m_fixed_seeds) {
            ...
            if (try_add_peer_locked(addr, DgbPeerInfo::Source::fixed_seed,
                                    /*require_routable=*/true))
                ++added;
        }
        if (added > 0) {
    ...
    void do_http_peer_fetch()
    {
        ...
        std::thread([seeds, symbol, self]() {
            ...
            // JSON key for the DGB chain feed.
            std::string key = "dgb";
    ...
    void add_http_peers(const std::vector<NetService>& peers)
    {
        ...
        for (auto& addr : peers) {
            ...
            if (try_add_peer_locked(addr, DgbPeerInfo::Source::addr_crawl,
                                    /*require_routable=*/true))
                ++added;
        }
        if (added > 0)
