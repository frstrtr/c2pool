...
// DGB PPLNS payout redistribution (Phase B B1, Scrypt-only).
// Ported from src/impl/ltc/redistribute.hpp into the dgb:: namespace. PPLNS
// redistribution math (PILLAR 2) uses the shared base and is byte-identical to the
// p2pool-merged-v36 reference, so it ports verbatim with only the namespace swap.
// Donation-script specifics (farsider350 2-of-3 vs LTC) live in donation_consensus
// and are HELD pending operator decision -- not resolved here.
