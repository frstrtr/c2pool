...
// DGB config aggregator. LTC defines this in config.hpp; the DGB tree splits
// config into config_pool.hpp + config_coin.hpp with no aggregator header, so
// the alias lives here (the sole consumer is the node template below).
...
public:
    NodeImpl()
        : m_coin_params(dgb::make_coin_params(false)),
    ...
    NodeImpl(boost::asio::io_context* ctx, config_t* config)
        : m_coin_params(dgb::make_coin_params(config->m_testnet)),
          base_t(ctx, config),
          m_share_getter(ctx,
            ...
            {
                auto rmsg = dgb::message_sharereq::make_raw(req_id, hashes, parents, stops);
    ...
    void request_shares(uint256 id, peer_ptr peer,
                        std::vector<uint256> hashes, uint64_t parents,
                        std::vector<uint256> stops,
                        std::function<void(dgb::ShareReplyData)> callback)
    {
    ...
    void got_share_reply(uint256 id, dgb::ShareReplyData shares)
    {
        try { m_share_getter.got_response(id, shares); }
        catch (const std::invalid_argument&) { /* request already timed out */ }
    }

    std::vector<dgb::ShareType> handle_get_share(std::vector<uint256> hashes, uint64_t parents, std::vector<uint256> stops, NetService peer_addr);
...
public:
    ...
    ADD_HANDLER(addrs, dgb::message_addrs);
    ADD_HANDLER(addrme, dgb::message_addrme);
    ADD_HANDLER(ping, dgb::message_ping);
    ADD_HANDLER(getaddrs, dgb::message_getaddrs);
    ADD_HANDLER(shares, dgb::message_shares);
    ADD_HANDLER(sharereq, dgb::message_sharereq);
    ADD_HANDLER(sharereply, dgb::message_sharereply);
    ADD_HANDLER(bestblock, dgb::message_bestblock);
    ADD_HANDLER(have_tx, dgb::message_have_tx);
    ADD_HANDLER(losing_tx, dgb::message_losing_tx);
    ADD_HANDLER(remember_tx, dgb::message_remember_tx);
    ADD_HANDLER(forget_tx, dgb::message_forget_tx);
};
