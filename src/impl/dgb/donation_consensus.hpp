...
#pragma once
//
// donation_consensus.hpp — Consensus-level donation script validation for DGB-Scrypt p2pool shares.
//
...
//
// Reference: frstrtr/p2pool-merged-v36 p2pool/data.py lines 114-200; DGB donation
// SOURCE OF TRUTH is the DGB oracle frstrtr/p2pool-dgb-scrypt (switch-oracle, Option B).
// DGB donation scripts (v35 forrestv P2PK 67B [4104ffd0...] / v36 combined P2SH 1-of-2) are
// owned by dgb::PoolConfig::get_donation_script in config_pool.hpp; this layer
// is share-version-agnostic and defers script selection there.
//
// Scrypt-only (V36): only Scrypt-validated shares reach this path; other DGB
// algos are accept-by-continuity per project_v36_dgb_scrypt_only.
//
...
//   3. Pre-V36 shares use the DGB v35 forrestv P2PK DONATION_SCRIPT (4104ffd0...);
//      V36+ shares use COMBINED_DONATION_SCRIPT (P2SH). Selection is done by
//      dgb::PoolConfig::get_donation_script(share_version).
//
// Parameters:
//   coinbase_outputs: all outputs from the coinbase transaction
//   expected_payouts: result of ShareTracker::get_expected_payouts()
//   share_version:    the share version number (selects donation script)
//
