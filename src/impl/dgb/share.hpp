...
// DGB Scrypt share definition (header+payload). Scrypt PoW; 80-byte header algo-agnostic.
//
// Ported from src/impl/ltc/share.hpp into the dgb:: namespace, Scrypt-only.
// LTC is itself a Scrypt parent and the V36 SHARE format is shared:
//   CONFIRMED against p2pool-merged-v36 networks/digibyte.py —
//   SEGWIT_ACTIVATION_VERSION=17 (== LTC), MINIMUM_PROTOCOL_VERSION=3301.
// The share wire-format is therefore byte-identical to LTC's; this port changes
// only the namespace. Any divergence from p2pool-merged-v36 => [decision-needed].
//
// Conformance constants (PPLNS, min-proto floor, AutoRatchet thresholds) live in
// the pool/config layer and stay fenced TODO stubs until the LTC soak freezes the
// reference — they are NOT part of this share definition.
...
struct BaseShare : chain::BaseShare<uint256, Version>
{
    ...
    dgb::StaleInfo m_stale_info;        // stale_info
...
struct Share : BaseShare<17>
{
    ...
    dgb::ShareTxInfo m_tx_info; // new_transaction_hashes; transaction_hash_refs
