...
// DGB sharechain tracker (Phase B B1, Scrypt-only).
// Ported from src/impl/ltc/share_tracker.hpp into the dgb:: namespace. LTC is the
// Scrypt parent, so PPLNS ring (PILLAR 2) + anti-hop/tail-guard (PILLAR 3) math is
// byte-identical to the p2pool-merged-v36 reference and ports verbatim. Fenced
// conformance constants (exact PPLNS, min-proto 3301, AutoRatchet 95/50/2x) live in
// share_check / config_pool, not here. COMPAT: frstrtr/p2pool-merged-v36; any
// divergence => [decision-needed].
// NOTE: depends on share_check.hpp (still a B1 stub; ported next this milestone).
...
public:
    ...
    // Coin parameters (non-owning). Set by the owning dgb::NodeImpl right
...
public:
    ...
    void scan_chain_for_blocks(const uint256& tip, int max_shares)
    {
        ...
        int scanned = 0, found_dgb = 0, found_merged = 0, no_pow = 0;
        uint256 pos = tip;
        for (int i = 0; i < max_shares && !pos.IsNull(); ++i) {
            ...
            chain.get(pos).share.invoke([&](auto* s) {
                ...
                if (!block_target.IsNull() && pow <= block_target) {
                    ...
                    if (m_on_block_found) { m_on_block_found(pos); ++found_dgb; }
                }
    ...
    struct MinerThresholds {
        ...
        double min_payout_dgb;       // LTC per share at pool difficulty
                                          ...
                                          uint64_t block_subsidy_sat)
    {
        ...
        t.min_payout_dgb = static_cast<double>(block_subsidy_sat) / 1e8 / chain_length;
    ...
    get_merged_expected_payouts(const uint256& best_share_hash,
                                ...
                                uint32_t chain_id,
                                const std::vector<unsigned char>& donation_script,
                                const std::vector<unsigned char>& operator_dgb_script = {},
                                const std::vector<unsigned char>& operator_merged_script = {})
    {
        ...
        for (const auto& [key, weight] : weights) {
            std::vector<unsigned char> script;
            if (!operator_dgb_script.empty() && !operator_merged_script.empty() && key == operator_dgb_script) {
