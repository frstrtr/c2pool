...
#include <impl/dgb/coin/aux_doge_mm_commitment.hpp>
...
// ConnCoinbasePplnsInputs appends the #475 build_aux_mm_commitment blob to the
...
//      grows the gentx by exactly 44 bytes; the tag bytes match the #475 SSOT.
TEST(ConnCoinbaseEmbedAtMint, AppendsMmTagToScriptSig) {
    ...
    auto tag = dgb::coin::build_aux_mm_commitment(aux_root, /*size=*/1, /*nonce=*/0);
    ASSERT_EQ(tag.size(), dgb::coin::AUX_MM_COMMITMENT_SIZE);
