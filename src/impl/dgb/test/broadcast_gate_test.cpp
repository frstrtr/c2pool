...
// ---------------------------------------------------------------------------
// F3 tx-completeness broadcast gate + F2 mark-only-what-was-sent — DGB KATs.
//
// Both primitives are the ones send_shares()/broadcast_share() in
// src/impl/dgb/node.cpp actually call, so these KATs pin the shipped policy:
//
...
TEST(DgbBroadcastMarking, NothingMarkedWhenNoPeerAccepted)
{
...
TEST(DgbBroadcastMarking, NothingMarkedWithZeroPeers)
{
    std::set<uint256> marked;
    std::map<uint64_t, FakePeer> peers;

    EXPECT_EQ(dgb::broadcast_and_mark(marked, peers, std::vector<uint256>{S1, S2},
                                      [](FakePeer&) { return std::vector<uint256>{}; }),
              0u);
    EXPECT_TRUE(marked.empty());
}
...
TEST(DgbBroadcastMarking, OnlyAcceptedHashesAreMarked)
{
    std::set<uint256> marked;
...
TEST(DgbBroadcastMarking, WalkStrandedWhenMarkedBeforeSend)
{
    ...
    // ---- pre-fix ordering: mark the walk, THEN discover nothing was sent ----
    {
        ...
        dgb::broadcast_and_mark(marked, one_peer, to_send,
                                [](FakePeer&) { return std::vector<uint256>{}; });
    ...
    // ---- shipped ordering: mark only what a peer accepted --------------------
    {
        ...
        dgb::broadcast_and_mark(marked, live, retry, [](FakePeer& p) {
            return std::vector<uint256>(p.accepts.begin(), p.accepts.end());
        });
