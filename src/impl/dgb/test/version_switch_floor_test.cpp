...
// Conformance KAT: version-switch 60% gate rounding (dgb::version_switch_underweight).
//
// The DGB oracle (p2pool-dgb-scrypt data.py:505) rejects an upgrade share when
...
TEST(DgbVersionSwitchFloor, NonMultipleBoundaryAcceptsFloor)
{
    const uint288 T(7);            // 60*7 = 420, floor(4.2) = 4
    EXPECT_FALSE(dgb::version_switch_underweight(uint288(4), T));
    EXPECT_TRUE(dgb::version_switch_underweight(uint288(3), T));
    // Divergence pin: the old form rejected the oracle-valid share.
    EXPECT_TRUE(old_cross_multiply_reject(uint288(4), T));
}

TEST(DgbVersionSwitchFloor, ExactMultipleBoundaryUnchanged)
{
    const uint288 T(10);           // 60*10 = 600, threshold exactly 6
    EXPECT_FALSE(dgb::version_switch_underweight(uint288(6), T));
    EXPECT_TRUE(dgb::version_switch_underweight(uint288(5), T));
    EXPECT_FALSE(old_cross_multiply_reject(uint288(6), T));
    EXPECT_TRUE(old_cross_multiply_reject(uint288(5), T));
}

TEST(DgbVersionSwitchFloor, WorkScaleBoundaryAcceptsFloor)
{
    ...
    EXPECT_FALSE(dgb::version_switch_underweight(floor, T));
    EXPECT_TRUE(dgb::version_switch_underweight(below, T));
