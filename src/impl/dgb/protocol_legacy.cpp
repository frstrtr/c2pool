...
void Legacy::HANDLER(addrme)
{
    ...
    // NETWORK address. The DGB canonical, p2pool-dgb-scrypt p2p.py:267, compares
    // `host == 127.0.0.1`, and so does the Actual generation
    // (protocol_actual.cpp:57). Legacy shipped "127.0.0.0", which no peer can
...
{
    try {
        dgb::HandleSharesData result; //share, txs
        ...
        for (auto wrappedshare : msg->m_shares)
        {
            ...
            try
            {
                share = dgb::load_share(wrappedshare, peer->addr());
            }
...
void Legacy::HANDLER(sharereq)
{
    ...
    try
    {
        ...
        auto reply_msg = message_sharereply::make_raw(msg->m_id, dgb::ShareReplyResult::good, rshares);
...
void Legacy::HANDLER(sharereply)
{
    ...
    if (msg->m_result == ShareReplyResult::good)
    {
        ...
        for (auto& rshare : msg->m_shares)
        {
            try
            {
                auto share = dgb::load_share(rshare, peer->addr());
