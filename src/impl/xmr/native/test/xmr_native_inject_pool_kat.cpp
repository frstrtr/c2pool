...
int main() {
    ...
    // ===================== daemon-first NAMED-REFUSAL ===================
    // The p2p-first arm always mines a 0-fee inject; the daemon-first arm must
    // refuse it BY NAME at submit time (monerod would reject the block), before
    // it can pin. Pure predicate, so it is pinned here without a node/rig.
    {
        using P = OperatorInjectPool;
        // p2p-first (daemon_first_arm == false): a 0-fee inject is ADMITTED.
        check(streq(P::daemon_first_refusal(false, true,  0), ""),
              "p2p-first admits a 0-fee inject (always mined)");
        check(streq(P::daemon_first_refusal(false, false, 0), ""),
              "p2p-first admits a 0-fee inject with no daemon too");
        // daemon-first + monerod configured + 0 fee: REFUSED by name.
        check(streq(P::daemon_first_refusal(true, true, 0),
                    "inject-daemon-first-0fee-unrelayable"),
              "daemon-first refuses a 0-fee inject by name");
        // daemon-first + monerod configured + a paying fee: ADMITTED.
        check(streq(P::daemon_first_refusal(true, true, 1), ""),
              "daemon-first admits a fee-paying inject");
        // daemon-first but NO monerod endpoint to judge it: ADMITTED (nothing
        // will reject the block, so the refusal does not apply).
        check(streq(P::daemon_first_refusal(true, false, 0), ""),
              "daemon-first with no daemon admits (no judge to reject)");
    }
