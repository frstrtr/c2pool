...
// ---------------------------------------------------------------------------
// BTC found-block confirm/orphan resolver -- TELEMETRY ONLY (#995/#1155 BTC arm).
//
...
// orphaned. Without it a BTC won block stays "pending" forever and an orphan
...
//
// BTC is a SHA256d parent: the block id == SHA256d(80-byte header), so the
// pow_hash/block_hash the miner found IS the block hash keyed here.
//
// This mirrors the DASH/BCH found-block lane CONTRACT
// (bch::coin::block_confirm) but is an INDEPENDENT BTC-tree implementation --
// per-coin isolation, no cross-coin code sharing.
// ---------------------------------------------------------------------------
#pragma once
...
namespace bip110::coin::block_confirm {
