...
#include "block.hpp"
#include <impl/bip110/pow.hpp>   // BLAKE2b block-identity + PoW pipeline (fork >= 961640)
...
struct BTCChainParams {
    ...
    // ── BIP-110 fork parameters ──────────────────────────────────────────────
    // At/after blake2b_height the chain switches from SHA256d to the BLAKE2b v2
    // header PoW (Knots kernel/chainparams.cpp:120-125). A one-off difficulty
    // reset shifts the target left by blake2b_target_shift at exactly that
    // height. Set to a sentinel (UINT32_MAX) on nets without the fork so the
    // shared SHA256d validation path is preserved unchanged.
    uint32_t blake2b_height{UINT32_MAX};       // first BLAKE2b (v2) block height
    uint32_t blake2b_target_shift{22};         // one-off target<<shift at the fork
    ...
    static BTCChainParams mainnet() {
        ...
        // BTC mainnet powLimit (Bitcoin Core CMainParams). Unchanged by BIP-110
        // (CheckProofOfWorkImpl target decode is untouched).
        p.pow_limit.SetHex("00000000ffff0000000000000000000000000000000000000000000000000000");
        // BIP-110: BLAKE2b hard fork at height 961640 with a one-off <<22 target
        // reset (Knots kernel/chainparams.cpp:120-125). Below this height the
        // chain is byte-identical to canonical Bitcoin (shared SHA256d prefix).
        p.blake2b_height = 961640;
        p.blake2b_target_shift = 22;
...
/// Compute the BIP-110 block-identity / PoW hash of a header.
/// Pre-fork (v1, bit31 clear, 80 bytes): SHA256d, shared with canonical Bitcoin.
/// At/after the fork (v2, bit31 set, 164 bytes): the BLAKE2b commitment pipeline
/// (bip110::pow::blake2b_block_hash_v2). The block identity == PoW value (like
...
/// Same dispatch as scrypt_hash: SHA256d below the fork, BLAKE2b at/after.
...
/// BIP-110 one-off difficulty reset (Knots pow.cpp:19-31 ApplyBlake2bTargetShift).
/// Applied at exactly blake2b_height: shift the target LEFT by target_shift
/// (easier difficulty), clamped to powLimit. If the pre-shift target already
/// exceeds powLimit>>shift, the shifted value would overflow powLimit, so it is
/// clamped to powLimit.
inline uint32_t apply_blake2b_target_shift(uint32_t base_bits, const BTCChainParams& params) {
    bool negative, overflow;
    uint256 target;
    target.SetCompact(base_bits, &negative, &overflow);
    if (negative || target.IsNull() || overflow)
        return base_bits;
    uint256 limit_shifted = params.pow_limit >> params.blake2b_target_shift;
    if (target > limit_shifted)
        target = params.pow_limit;
    else
        target <<= params.blake2b_target_shift;
    return target.GetCompact();
}

inline uint32_t get_next_work_required_base(
...
/// Difficulty for the block at tip_height+1. Identical to Bitcoin's retarget
/// EXCEPT at the BIP-110 fork height, where the one-off <<shift target reset is
/// applied on top of the normally-computed bits (Knots pow.cpp:70-73).
inline uint32_t get_next_work_required(
    ...
    const BTCChainParams& params)
{
    uint32_t base = get_next_work_required_base(get_ancestor, tip_height, tip_bits, tip_time, new_time, params);
    uint32_t new_height = tip_height + 1;
    if (new_height == params.blake2b_height)
        return apply_blake2b_target_shift(base, params);
    return base;
}
...
private:
    ...
    bool add_header_internal(const BlockHeaderType& header) {
        ...
        // A null hash means the BLAKE2b pipeline refused this header's layout
        // (unsupported flags / non-null xor_key). Reject — never index it.
        if (bhash.IsNull()) {
            LOG_WARNING << "[EMB-BIP110] REJECT header: unsupported v2 layout (null hash)";
            return false;
        }
        ...
        // ── BIP-110 fork version/height gate (UNCONDITIONAL, before PoW) ─────────
        // This is what makes the node FOLLOW the BLAKE2b chain instead of the
        // higher-work canonical SHA256d chain. It must run on every header, never
        // behind the bulk-sync structural-skip below.
        //   • height >= blake2b_height  ⇒ header MUST be v2 (bit31 set); a SHA256d
        //     header past the fork is foreign PoW → reject (bad-version-blake2b,
        //     Knots validation.cpp:4777-4784).
        //   • height <  blake2b_height  ⇒ header MUST be v1 (bit31 clear)
        //     (bad-version-sha256d, validation.cpp:4411-4416).
        //   • for a v2 header the committed m_height MUST equal new_height — the
        //     height is bound into the PoW h1 preimage (bad-header-height,
        //     validation.cpp:4778-4780), so this also pins PoW to the height.
        {
            const bool is_v2 = header.is_v2();
            const bool fork_active = (new_height >= m_params.blake2b_height);
            if (fork_active && !is_v2) {
                LOG_WARNING << "[EMB-BIP110] REJECT bad-version-blake2b at height=" << new_height
                            << " (SHA256d header past the fork) hash=" << bhash.GetHex().substr(0, 16);
                return false;
            }
            if (!fork_active && is_v2) {
                LOG_WARNING << "[EMB-BIP110] REJECT bad-version-sha256d at height=" << new_height
                            << " (v2 header before the fork) hash=" << bhash.GetHex().substr(0, 16);
                return false;
            }
            if (is_v2 && static_cast<uint32_t>(header.m_height) != new_height) {
                LOG_WARNING << "[EMB-BIP110] REJECT bad-header-height at height=" << new_height
                            << " committed=" << header.m_height << " hash=" << bhash.GetHex().substr(0, 16);
                return false;
            }
        }
