...
#include <impl/bip110/config_pool.hpp>
#include <impl/bip110/coin/softfork_check.hpp>
#include <impl/bip110/coin/genesis.hpp>       // btc_genesis_hash — per-net check() probe (#744/#787 B1)
...
NodeRPC::NodeRPC(io::io_context* context, bip110::interfaces::Node* coin, bool testnet)
...
bool NodeRPC::check()
{
	...
	if (blockchaininfo.contains("softforks"))
		bip110::coin::collect_softfork_names(blockchaininfo["softforks"], softforks_supported);
	if (blockchaininfo.contains("bip9_softforks"))
		bip110::coin::collect_softfork_names(blockchaininfo["bip9_softforks"], softforks_supported);
	...
	for (const auto& req : bip110::PoolConfig::SOFTFORKS_REQUIRED)
	{
...
rpc::WorkData NodeRPC::getwork()
{
	...
	std::vector<bip110::coin::Transaction> unpacked_transactions;
    for (auto& packed_tx : work["transactions"])
    {
        ...
        bip110::coin::MutableTransaction unpacked_tx;
        if (m_coin->known_txs.contains(txid))
        {
			unpacked_tx = bip110::coin::MutableTransaction(m_coin->known_txs.at(txid));
        } else
        {
            if (ps_tx.empty())
                ps_tx = PackStream(ParseHex(x));
        ...
        unpacked_transactions.push_back(bip110::coin::Transaction(unpacked_tx));
    }
...
void NodeRPC::submit_block(BlockType& block, bool ignore_failure)
{
	...
	PackStream packed_block = pack<bip110::coin::BlockType>(block);
	auto result = m_client.CallMethod<nlohmann::json>(ID, "submitblock", {HexStr(packed_block.get_span())});
	bool success = result.is_null();

	auto block_header = pack<bip110::coin::BlockHeaderType>(block); // cast to header?
