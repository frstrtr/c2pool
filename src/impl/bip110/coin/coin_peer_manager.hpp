...
/// BTC-ISOLATED multi-peer manager for the embedded coin-daemon P2P arm.
///
/// This is the network-standalone gate for a fully daemonless c2pool-btc:
/// it lets an embedded Bitcoin-network arm reach the BTC network on its OWN
/// scored, group-diverse peer set — independent of the local bitcoind — so
...
/// and the precondition for disabling the external bitcoind entirely.
///
...
/// the sibling per-coin CoinPeerManager copies (dash/dgb), NOT a reuse of the shared
/// merged tree. Each single-coin leaf keeps its own copy to preserve the
/// per-coin isolation fence (BTC lives entirely under src/impl/btc, no coupling
...
///
/// Header-only to match the sibling btc coin leaves (coin/*.hpp).
...
struct BtcPeerInfo
{
    ...
    bool                is_protected{false};    // local bitcoind node — never drop
    ...
    int compute_score() const
    {
        ...
        // This is what pulls the embedded arm OFF the local bitcoind's peer view
...
struct BtcPeerManagerConfig
{
    ...
    std::set<uint16_t> valid_ports;             // only connect to peers on known ports (BTC: 12024/12026)
...
// ─── BtcCoinPeerManager ──────────────────────────────────────────────────────
class BtcCoinPeerManager
{
public:
    using GetPeerInfoFn = std::function<std::vector<NetService>()>;

    BtcCoinPeerManager(boost::asio::io_context& ioc,
                       const std::string& symbol,
                       const std::string& data_dir,
                       const BtcPeerManagerConfig& config)
        : m_ioc(ioc)
        , m_symbol(symbol)
        , m_data_dir(data_dir)
        , m_config(config)
    ...
    ~BtcCoinPeerManager() { stop(); }

    /// Set callback to get peers from bitcoind via RPC getpeerinfo (optional).
    void set_getpeerinfo_fn(GetPeerInfoFn fn) { m_getpeerinfo_fn = std::move(fn); }

    /// Register the protected/pinned local bitcoind node. Score=999999, never
    ...
    bool set_local_node(const NetService& addr)
    {
        ...
        LOG_INFO << "[" << m_symbol << "] Protected local bitcoind node (pinned): " << key
    ...
    void start()
    {
        ...
        LOG_INFO << "[" << m_symbol << "] BtcCoinPeerManager started: "
...
private:
                             ...
                             const std::string& source_host = std::string())
    {
        ...
        // don't re-add the bitcoind's own peers under a preferred source.
        auto key = ep->to_string();
        if (m_peers.count(key)) return false;
        if (source != BtcPeerInfo::Source::coind && m_coind_peers.count(key)) return false;

        // ── Network group limits (Sybil resistance) ──
        auto group = peer_network_group(ep->host());
        bool is_untried_source = (source == BtcPeerInfo::Source::addr_crawl
                                || source == BtcPeerInfo::Source::dns_seed
                                || source == BtcPeerInfo::Source::fixed_seed);
    ...
    void bootstrap_from_getpeerinfo()
    {
        ...
        try {
            ...
            for (auto& addr : peers) {
                ...
                if (try_add_peer_locked(addr, BtcPeerInfo::Source::coind,
    ...
    std::string db_path() const
    {
        ...
        if (m_data_dir.empty() || m_data_dir == ".")
            dir = ::core::filesystem::config_path() / "btc_embedded_peers";
        else
            dir = m_data_dir;
        ...
        if (!clean) {
            LOG_WARNING << "[BtcPeerManager] non-alphanumeric coin symbol '"
        ...
        try {
            ...
            if (j.contains("peers") && j["peers"].is_object()) {
                ...
                for (auto& [key, pj] : j["peers"].items()) {
                    ...
                    peer.source = static_cast<BtcPeerInfo::Source>(
                        pj.value("source", 0));
    ...
    void bootstrap_from_dns_seeds()
    {
        ...
        for (auto& addr : peers) {
            ...
            if (try_add_peer_locked(addr, BtcPeerInfo::Source::dns_seed,
    ...
    void load_fixed_seeds()
    {
        ...
        for (auto& addr : m_fixed_seeds) {
            ...
            if (try_add_peer_locked(addr, BtcPeerInfo::Source::fixed_seed,
                                    /*require_routable=*/true))
                ++added;
        }
        if (added > 0) {
    ...
    void add_http_peers(const std::vector<NetService>& peers)
    {
        ...
        for (auto& addr : peers) {
            ...
            if (try_add_peer_locked(addr, BtcPeerInfo::Source::addr_crawl,
                                    /*require_routable=*/true))
                ++added;
        }
        if (added > 0)
