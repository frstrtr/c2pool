...
// Legacy V1 serialization — superseded by pack.hpp for V36 shares.
...
struct Maker : BaseMaker
{
    ...
    template <typename LAST_T>
    static std::vector<T> make_list_type(std::vector<LAST_T> v_arr)
    {
        std::vector<T> res;
        for (auto v : v_arr)
        {
...
{
    static std::vector<LIST_TYPE> make_type(std::vector<LIST_TYPE> values)
    {
        return values;
    }
};
...
struct MakerListType<LIST_TYPE> : BaseMaker
{
    ...
    static std::vector<LIST_TYPE> make_type(std::vector<SUB_EL_TYPE> values)
    {
...
struct PackStream
{
    ...
    PackStream(std::vector<unsigned char> value)
    {
        data = value;
    }
    ...
    {
        data = std::vector<unsigned char>(value, value + len);
    }
    ...
    PackStream(char *value, int32_t len)
    {
        ...
        data = std::vector<unsigned char>(temp, temp + len);
    }
...
public:
    ...
    std::shared_ptr<ValueType> get()
    {
        if (!_value)
        {
            ...
            {
                // TODO: LOG_DEBUG_OTHER << "StreamTypeAdapter operator -> warning: _value and _stream - nullptr!";
            }
        }
        return _value;
    }
