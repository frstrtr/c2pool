...
struct ListType : public MakerListType<T>, public GetterList<T>
{
    ...
    ListType(std::vector<T> arr)
    {
		GetterList<T>::value = std::vector<T>(arr);
    }

    auto &operator=(std::vector<T> _value)
    {
...
struct StrType : public Maker<StrType, std::string>, public CustomGetter<std::string>
{
    ...
    StrType(std::string _str)
    {
        value.insert(value.begin(), _str.begin(), _str.end());
    }

    StrType(std::vector<unsigned char> _c_str)
    {
        value = _c_str;
    }
...
template <int SIZE>
struct FixedStrType : public Maker<FixedStrType<SIZE>, std::string>, public CustomGetter<std::string>
{
    std::vector<unsigned char> value;

    FixedStrType() = default;

    FixedStrType(std::string _str)
    {
        if (_str.length() != SIZE)
        {
    ...
    FixedStrType(std::vector<unsigned char> _c_str)
    {
        if (_c_str.size() != SIZE)
        {
            throw packstream_exception("Incorrect length str in FixedStrType");
