...
struct AddrValue
{
    ...
    AddrValue() {}
    AddrValue(uint64_t service, uint64_t first_seen, uint64_t last_seen) : m_service(service), m_first_seen(first_seen), m_last_seen(last_seen) { }
...
struct AddrStorePair
{
    ...
    friend bool operator==(const AddrStorePair& l, const AddrStorePair& r)
    { return (l.addr == r.addr) && (std::tie(l.value.m_first_seen, l.value.m_last_seen, l.value.m_service) == std::tie(r.value.m_first_seen, r.value.m_last_seen, r.value.m_service)); }
...
public:
    ...
    void update(const NetService& addr, AddrValue new_value);
    // for bootstrap
