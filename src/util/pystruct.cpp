...
namespace c2pool::messages::python
{
    ...
    auto pymessage2::GetMethodObject(const char *method_name, const char *filename)
    {
        c2pool::python::Py::Initialize();
    ...
    char * pymessage2::GetCallFunctionResult(PyObjectType *pyObj)
    {
        char *ret = NULL;
        if (pyObj != NULL)
        {
    ...
    template <typename T>
    char *pymessage2::serialize(char *name_type, T &value)
    {
    ...
    char *pymessage2::serialize(c2pool::messages::message *msg)
    { //TODO: test * - > &
        return serialize(msg->command, *msg);
    }

    UniValue pymessage2::deserialize(char *name_type, char *value, int length)
    {
    ...
    UniValue pymessage2::deserialize(c2pool::messages::message *msg)
    {
        UniValue result(UniValue::VOBJ);
        auto methodObj = GetMethodObject("deserialize");
        if (methodObj == nullptr)
    ...
    int pymessage2::payload_length(c2pool::messages::message *msg){
        int result;
        std::stringstream ss;
    ...
    template <typename T>
    unsigned int pymessage2::packed_size(char *name_type, T &value)
    {
    ...
    unsigned int pymessage2::receive_length(char *length_data)
    {
        c2pool::python::Py::Initialize();
