...
{
    //--------------------------Factory
    Factory::Factory(boost::asio::io_context &context, std::shared_ptr<c2pool::p2p::NodesManager> _nodes) : io_context(context)
    {
        nodes = _nodes;
    }

    void Factory::protocol_connected(std::shared_ptr<c2pool::p2p::Protocol> proto)
    {
    ...
    //--------------------------Client
    Client::Client(boost::asio::io_context &io_context_, std::shared_ptr<c2pool::p2p::NodesManager> _nodes, int _desired_conns, int _max_attempts) : resolver(io_context_), Factory(io_context_, _nodes), _think_timer(_nodes->io_context(), boost::posix_time::seconds(0))
    {
    ...
    //--------------------------Server
    Server::Server(boost::asio::io_context &io_context_, std::shared_ptr<c2pool::p2p::NodesManager> _nodes, const tcp::endpoint &endpoint, int _max_conns)
