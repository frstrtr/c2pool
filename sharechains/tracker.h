...
public:
								...
								UniValue other_data = UniValue())
	{
		...
		{
			auto attempts_per_second = get_pool_attempts_per_second(share_data.previous_share_hash,
																	net->TARGET_LOOKBEHIND, true);
			//TODO:
	...
	*/
		...
		if (previous_share != nullptr)
		{
			...
			{
				result_timestamp = std::clamp(desired_timestamp, previous_share->timestamp + 1,
