...
void Share::check(ShareTracker* _tracker, std::optional<std::map<uint256, coind::data::tx_type>> other_txs)
{
    auto start = c2pool::dev::debug_timestamp();
...
//    LOG_INFO << "\tCHECK TIME: " << final-start;
//    LOG_INFO << "\t\t" << "t1-start:" << t1-start;
